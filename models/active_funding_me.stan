...
parameters {
  ...
  real theta_share;
  real beta; // trend
  vector[R] beta_r; // country-specific trend
  real tau_beta; // variation in country-specific trend
  ...
  // loop model over countries
  {
    ...
    for (r in 1:R) {
      for (i in 1:N_target[r]) {
        ...
        mu_hiv_new[counter_target] = uninfected_me[counter_target] * hiv_living_me[counter_target] * exp( alpha + alpha_r[r] + (beta + beta_r[r]) * i + (1 / 1e3 * sum(active_funding[counter_target]) / hiv_living[counter_data]) * (theta) + theta_share * active_funding[counter_target,1] / sum(active_funding[counter_target]) + Z[counter_data] * zeta);
...
model {
  ...
  beta_r ~ normal(0., tau_beta);
  tau_beta ~ student_t(4., 0., 1.);
  phi ~ normal(0., 1.);
  ...
  real mu_hiv_new_funding[max_N_target,R] ;
  {
    ...
    for (r in 1:R) {
      for (i in 1:N_target[r]) {
        ...
        mu_hiv_new_funding[i,r] = uninfected_me[counter_target] * hiv_living_me[counter_target] * exp( alpha + alpha_r[r] + (beta + beta_r[r]) * i + (1 / 1e3 * sum(mu_active_funding[counter_target]) / hiv_living_me[counter_target]) * (theta) + theta_share * mu_active_funding[counter_target,1] / sum(mu_active_funding[counter_target]) + Z[counter_data] * zeta );
      }
    }
  }
}