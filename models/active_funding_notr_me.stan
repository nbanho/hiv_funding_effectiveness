functions {
  ...
  real diff_lnorm(real x, real max_x, real mu, real sigma) {
    {
      real log_mu = log( mu ^ 2 / sqrt( sigma ^ 2 + mu ^ 2 ) );
      real log_sigma = sqrt( log( sigma ^ 2 / mu ^ 2 + 1 ) );
      if (x == 0) {
        return lognormal_cdf(1,  log_mu, log_sigma) / lognormal_cdf(max_x, log_mu, log_sigma);
      } else {
        return (lognormal_cdf(x+1, log_mu, log_sigma) - lognormal_cdf(x, log_mu, log_sigma)) / lognormal_cdf(max_x, log_mu, log_sigma);
      }
    }
  }
  ...
  // loop model over countries
  {
    ...
    for (r in 1:R) {
      for (i in 1:N_target[r]) {
        ...
        // - with country-specific funding effect: + theta_r[r] 
        mu_hiv_new[counter_target] = uninfected_me[counter_target] * hiv_living_me[counter_target] * exp( alpha + alpha_r[r] + (1 / 1e3 * sum(active_funding[counter_target]) / hiv_living[counter_data]) * (theta) + theta_share * active_funding[counter_target,1] / sum(active_funding[counter_target]) + Z[counter_data] * zeta);
...
model {
  ...
  // - with year RE: alpha_y ~ normal(0., tau[2]);
  // - with country-specific funding effect: theta_r ~ normal(0., tau[2]);
  tau ~ student_t(4., 0., 1.);
  // - with year RE: tau[2] ~ student_t(4., 0., 1.);
  // - with country-specific funding effect: tau[3] ~ student_t(4., 0., 1.);
  phi ~ normal(0., 1.);
...
generated quantities {
  ...
  // expected amount of active funding
  matrix[max_N_target,R] active_funding_tot;
  // expected number of new infections with and without funding
  matrix[max_N_target,R] mu_hiv_new_none;
  matrix[max_N_target,R] mu_hiv_new_funding ;
  {
    int counter_data = 0; // counter for the input data an
    int counter_target = 0; // counter for modeled data
    for (r in 1:R) {
      for (i in 1:N_target[r]) {
        ...
        // expected amount of active funding
        active_funding_tot[i,r] = (1 / 1e3 * sum(mu_active_funding[counter_target]) / hiv_living_me[counter_target]);
        ...
        mu_hiv_new_funding[i,r] = uninfected_me[counter_target] * hiv_living_me[counter_target] * exp( alpha + alpha_r[r] + active_funding_tot[i,r] * (theta) + Z[counter_data] * zeta );
      }
    }
  }
}
