...
parameters {
  ...
  real<lower=0> tau_alpha; // between-country variation for alpha
  real beta; // trend
  vector[R] beta_r; // country-specific trend
  real<lower=0> tau_beta; // between-country variation for beta
  ...
  // loop model over countries
  {
    ...
    for (r in 1:R) {
      for (i in 1:N_target[r]) {
        ...
        mu_hiv_new[counter_target] = uninfected[counter_data] * hiv_living_me[counter_target] * exp( alpha + alpha_r[r] + (beta + beta_r[r]) * i + Z[counter_data] * zeta);
